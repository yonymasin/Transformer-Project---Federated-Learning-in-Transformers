    ...
    if args.alg == 'fedavg':
        ...
        for round in range(args.comm_round):
            ...
            if args.dataset == 'shakespeare':
                ...
                local_train_net_per(nets, selected, args, net_dataidx_map_train, net_dataidx_map_test, logger,
                                    device=device)
        ...
        for round in range(args.comm_round):
            ...
            if (round + 1) >= test_round and (round + 1) % eval_step == 0:
                if args.dataset == 'shakespeare':
                    ...
                    train_results, train_avg_loss, train_acc, train_all_acc, test_results, test_avg_loss, test_acc, test_all_acc = compute_accuracy_per_client(
                        hnet, nets, global_model, args, net_dataidx_map_train, net_dataidx_map_test, num_class,
                        device=device)
        ...
        for round in range(args.comm_round):
            ...
            if args.dataset == 'shakespeare':
                local_train_net_fedprox(nets, selected, global_model, args, train_dl_global, test_dl_global, logger,
                                        device=device)
            else:
                local_train_net_fedprox(nets, selected, global_model, args, net_dataidx_map_train, net_dataidx_map_test,
        ...
        for round in range(args.comm_round):
            ...
            if args.dataset == 'shakespeare':
                update_dict = local_train_net_pfedMe(nets, selected, global_model, args, train_dl_global,
                                                     test_dl_global, logger, device=device)
            else:
                update_dict = local_train_net_pfedMe(nets, selected, global_model, args, net_dataidx_map_train,
                                                     net_dataidx_map_test, logger, device=device)
        ...
        for round in range(args.comm_round):
            ...
            if round == 0:
                for iidx in range(args.n_parties):
                    ...
                    if args.dataset == "shakespeare":
                        if args.model == 'lstm':
                            personalized_pred_list[iidx]["decoder.weight"] = copy.deepcopy(
                                final_state["decoder.weight"])
                            personalized_pred_list[iidx]["decoder.bias"] = copy.deepcopy(final_state["decoder.bias"])
                        elif args.model == 'transformer':
                            personalized_pred_list[iidx]["trg_word_prj.weight"] = copy.deepcopy(
                                final_state["trg_word_prj.weight"])
                    else:
                        if args.model == 'cnn':
                            ...
                            personalized_pred_list[iidx]["mlp_head.1.weight"] = copy.deepcopy(
                                final_state["mlp_head.1.weight"])
                            personalized_pred_list[iidx]["mlp_head.1.bias"] = copy.deepcopy(
                                final_state["mlp_head.1.bias"])
            ...
            for idx in range(len(selected)):
                if round != 0:
                    ...
                    if args.dataset == "shakespeare":
                        if args.model == 'lstm':
                            personalized_pred_list[selected[idx]]["decoder.weight"] = copy.deepcopy(
                                final_state["decoder.weight"])
                            personalized_pred_list[selected[idx]]["decoder.bias"] = copy.deepcopy(
                                final_state["decoder.bias"])
                        elif args.model == 'transformer':
                            personalized_pred_list[selected[idx]]["trg_word_prj.weight"] = copy.deepcopy(
                                final_state["trg_word_prj.weight"])
                    else:
                        if args.model == 'cnn':
                            ...
                            personalized_pred_list[selected[idx]]["mlp_head.1.weight"] = copy.deepcopy(
                                final_state["mlp_head.1.weight"])
                            personalized_pred_list[selected[idx]]["mlp_head.1.bias"] = copy.deepcopy(
                                final_state["mlp_head.1.bias"])
        ...
        for round in range(args.comm_round):
            ...
            update_dict = local_train_net_fedRod(nets, selected, personalized_pred_list, args,
                                                 net_dataidx_map_train, net_dataidx_map_test, logger, alpha=alpha_dict,
                                                 device=device)
            ...
            if (round + 1) >= test_round and (round + 1) % eval_step == 0:
                train_results, train_avg_loss, train_acc, train_all_acc, test_results, test_avg_loss, test_acc, test_all_acc = compute_accuracy_perRod(
                    personalized_pred_list, global_model, args, net_dataidx_map_train, net_dataidx_map_test, alpha_dict,
                    device=device)
        ...
        accessories = args.alg + "-linear-" + str(args.n_parties) + "-" + str(
            args.dataset) + "-" + args.partition + "-" + args.comment
        ...
        for round in range(args.comm_round):
            ...
            if round == 0:
                for iidx in range(args.n_parties):
                    final_state = copy.deepcopy(nets[iidx].state_dict())
                    for ll in range(4):
                        personalized_bn_list[iidx]["bn" + str(ll + 1) + ".weight"] = copy.deepcopy(
                            final_state["bn" + str(ll + 1) + ".weight"])
                        personalized_bn_list[iidx]["bn" + str(ll + 1) + ".bias"] = copy.deepcopy(
                            final_state["bn" + str(ll + 1) + ".bias"])

            for idx in range(len(selected)):
                if round != 0:
                    final_state = copy.deepcopy(nets[selected[idx]].state_dict())
                    for ll in range(4):
                        personalized_bn_list[selected[idx]]["bn" + str(ll + 1) + ".weight"] = copy.deepcopy(
                            final_state["bn" + str(ll + 1) + ".weight"])
                        personalized_bn_list[selected[idx]]["bn" + str(ll + 1) + ".bias"] = copy.deepcopy(
                            final_state["bn" + str(ll + 1) + ".bias"])
            ...
            if (round + 1) >= test_round and (round + 1) % eval_step == 0:
                ...
                train_results, train_avg_loss, train_acc, train_all_acc, test_results, test_avg_loss, test_acc, test_all_acc = compute_accuracy_personally(
                    personalized_bn_list, global_model, args, net_dataidx_map_train, net_dataidx_map_test,
                    device=device)
        ...
        for round in range(args.comm_round):
            ...
            if round == 0:
                for iidx in range(args.n_parties):
                    ...
                    for ll in range(args.depth):
                        if args.dataset == "shakespeare":
                            personalized_kqv_list[iidx][
                                "encoder.layer_stack." + str(ll) + ".slf_attn.w_qs.weight"] = copy.deepcopy(
                                final_state["encoder.layer_stack." + str(ll) + ".slf_attn.w_qs.weight"])
                            personalized_kqv_list[iidx][
                                "encoder.layer_stack." + str(ll) + ".slf_attn.w_ks.weight"] = copy.deepcopy(
                                final_state["encoder.layer_stack." + str(ll) + ".slf_attn.w_ks.weight"])
                            personalized_kqv_list[iidx][
                                "encoder.layer_stack." + str(ll) + ".slf_attn.w_vs.weight"] = copy.deepcopy(
                                final_state["encoder.layer_stack." + str(ll) + ".slf_attn.w_vs.weight"])
                        else:
                            personalized_kqv_list[iidx][
                                "transformer.layers." + str(ll) + ".0.fn.to_qkv.weight"] = copy.deepcopy(
                                final_state["transformer.layers." + str(ll) + ".0.fn.to_qkv.weight"])
            ...
            for idx in range(len(selected)):
                if round != 0:
                    ...
                    for ll in range(args.depth):
                        if args.dataset == "shakespeare":
                            personalized_kqv_list[selected[idx]][
                                "encoder.layer_stack." + str(ll) + ".slf_attn.w_qs.weight"] = copy.deepcopy(
                                final_state["encoder.layer_stack." + str(ll) + ".slf_attn.w_qs.weight"])
                            personalized_kqv_list[selected[idx]][
                                "encoder.layer_stack." + str(ll) + ".slf_attn.w_ks.weight"] = copy.deepcopy(
            ...
            if (round + 1) >= test_round and (round + 1) % eval_step == 0:
                if args.dataset == "shakespeare":
                    ...
                    train_results, train_avg_loss, train_acc, train_all_acc, test_results, test_avg_loss, test_acc, test_all_acc = compute_accuracy_personally(
                        personalized_kqv_list, global_model, args, net_dataidx_map_train, net_dataidx_map_test,
                        device=device)
        ...
        for round in range(args.comm_round):
            ...
            if (round + 1) >= test_round and (round + 1) % eval_step == 0:
                train_results, train_avg_loss, train_acc, train_all_acc, test_results, test_avg_loss, test_acc, test_all_acc = compute_accuracy_hphead_client(
                    hnet, personalized_pred_list, global_model, args, net_dataidx_map_train, net_dataidx_map_test,
                    device=device)
