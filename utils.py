    ...
    for net_id in range(args.n_parties):
        ...
        if args.dataset == 'shakespeare':
            ...
            if args.k_neighbor:
                ...
                test_correct, test_total, test_avg_loss = compute_accuracy_loss_knn(local_model, train_dl_local,
                                                                                    test_dl_local, datastore, vec_dim,
                                                                                    args, device=device)
                if args.train_acc_pre:
                    train_correct, train_total, train_avg_loss = compute_accuracy_loss_knn(local_model, train_dl_local,
                                                                                           train_dl_local, datastore,
                                                                                           vec_dim, args, device=device)
...
def compute_accuracy_percnn_client(hyper, global_model, args, net_dataidx_map_train, net_dataidx_map_test,
                                   device="cpu"):
    hyper.eval()
    if args.train_acc_pre:
        train_results = defaultdict(lambda: defaultdict(list))
...
def compute_accuracy_personally(personal_qkv_list, global_model, args, net_dataidx_map_train, net_dataidx_map_test,
                                device="cpu"):
    if args.train_acc_pre:
        train_results = defaultdict(lambda: defaultdict(list))
    test_results = defaultdict(lambda: defaultdict(list))
...
def compute_accuracy_perRod(personal_head_list, global_model, args, net_dataidx_map_train, net_dataidx_map_test,
                            alpha_dict, device="cpu", hyper=None):
    if hyper != None:
        hyper.eval()
    if args.train_acc_pre:
        train_results = defaultdict(lambda: defaultdict(list))
    ...
    for net_id in range(args.n_parties):
        ...
        if args.dataset == "shakespeare":
            ...
            test_correct, test_total, test_avg_loss = compute_accuracy_fedRod(local_model, p_head, test_dl_local,
                                                                              sample_per_class, args, device=device)
            if args.train_acc_pre:
                train_correct, train_total, train_avg_loss = compute_accuracy_fedRod(local_model, p_head,
                                                                                     train_dl_local, sample_per_class,
                                                                                     args, device=device)
...
def compute_accuracy_hphead_client(hyper, personal_qkv_list, global_model, args, net_dataidx_map_train,
                                   net_dataidx_map_test, device="cpu"):
    hyper.eval()
    if args.train_acc_pre:
        train_results = defaultdict(lambda: defaultdict(list))
    test_results = defaultdict(lambda: defaultdict(list))
