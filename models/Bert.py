...
class BertForSequenceClassification(nn.Module):
    ...
    def reduce_seq_len(self, seq_len):
        ...
        self.bert.embeddings.position_embeddings.weight = nn.Parameter(
            self.bert.embeddings.position_embeddings.weight[:seq_len])
    ...
    def from_pretrained(cls, model_type, config_args=None, adaptive_weight_copy=False):
        ...
        model_hf = HFBertForSequenceClassification.from_pretrained(pretrained_model_name_or_path=model_type,
        ...
        for k in sd_keys_hf:
            ...
            if not adaptive_weight_copy:
                ...
                assert sd_hf[k].shape == sd[
                    k].shape, f"Shape mismatch: {k} --> (hf vs custom) ({sd_hf[k].shape} vs {sd[k].shape})"
